...
type AnalysisSummary = {
  biomarkers: { name: string; value: string; normalRange: string; status: string }[];
  summary: string;
};
...
export const AnalysisCourse = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (!['image/jpeg', 'image/png', 'application/pdf'].includes(uploadedFile.type)) {
        ...
        <div className='mb-6'>
          ...
          <label className='block'>
            ...
            <motion.div
              ...
              whileTap={{ scale: 0.98 }}
            >
              <IoDocumentAttachOutline size={24} className='mx-auto mb-2 text-blue-600' />
        ...
        <>
          ...
          {analysisSummary && (
            <div className='bg-white rounded-xl p-4 shadow-sm border border-gray-300 mb-6'>
              ...
              <p className='text-gray-600 text-sm mb-2'>{analysisSummary.summary}</p>
              {analysisSummary.biomarkers.length > 0 && (
                <ul className='text-gray-600 text-sm'>
                  {analysisSummary.biomarkers.map((bm, idx) => (
                    <li key={idx} className='mb-1'>
                      <strong>{bm.name}:</strong> {bm.value} ({bm.normalRange}, {bm.status})
                    </li>
                  ))}
                </ul>
              )}

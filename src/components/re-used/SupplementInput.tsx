...
type SupplementInputProps = {
  ...
  onRemove: (supplement: string) => void; // Добавляем callback для удаления
};
...
export const SupplementInput = ({ onAdd, onRemove }: SupplementInputProps) => {
  ...
  const [manualSupplements, setManualSupplements] = useState<string[]>([]); // Для добавленных вручную
  ...
  const handleAddSupplement = () => {
    if (name.trim()) {
      ...
      setManualSupplements([...manualSupplements, name.trim()]);
      setName('');
    }
  };
  ...
  const handleQuickAdd = (supplement: string) => {
    if (selectedSupplements.includes(supplement)) {
      // Если добавка уже выбрана, удаляем её
      onRemove(supplement);
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      // Имитация распознавания названий (заглушка)
      newPhotos.forEach((file, index) => {
        const supplementName = `Добавка ${photos.length + index + 1}`;
        onAdd({ name: supplementName });
        setSelectedSupplements([...selectedSupplements, supplementName]);
      });
    }
  };
  ...
  const removePhoto = (index: number) => {
    ...
    const removedSupplement = `Добавка ${index + 1}`;
    setPhotos(updatedPhotos);
    setSelectedSupplements(selectedSupplements.filter(s => s !== removedSupplement));
    onRemove(removedSupplement);
  };

  const removeManualSupplement = (supplement: string) => {
    setManualSupplements(manualSupplements.filter(s => s !== supplement));
    setSelectedSupplements(selectedSupplements.filter(s => s !== supplement));
    onRemove(supplement);
  };
          ...
          {manualSupplements.length > 0 && (
            <div className='mb-4'>
              ...
              <div className='grid grid-cols-3 gap-2'>
                {manualSupplements.map((supplement, index) => (
                  <div
                    key={index}
